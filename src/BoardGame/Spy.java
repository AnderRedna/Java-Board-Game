...
public class Spy extends Piece {
    ...
    public Spy(String name, int xp, int yp, int level, String type, LinkedList<Piece> ps) {
    ...
    public void move(int xp, int yp, String type) {
        if (type == "player") {
            if ((Math.abs(xp - this.xp) == 1 && yp == this.yp)
                    || (xp == this.xp && Math.abs(yp - this.yp) == 1 && yp - this.yp == 1)) {
                if (ChessGame.getPiece(xp * 64, yp * 64) != null) {
                    if (ChessGame.getPiece(xp * 64, yp * 64).type != type
                            && ChessGame.getPiece(xp * 64, yp * 64).type != "river") {
